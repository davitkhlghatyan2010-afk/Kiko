/* 301AD pixel world — real pixel data in a small buffer, integer-scaled only. */
(function () {
  var PAL = {
  ...
  function lake(p, x0, y0, w, h, T, frame, treeXs) {
    p.rect(x0, y0, w, h, T.water);
    p.hl(x0, y0, w, T.waterL); p.hl(x0, y0 + 1, w, T.waterL);
    if (treeXs) treeXs.forEach(function (tx) {
      if (tx > x0 && tx < x0 + w) for (var y = y0 + 2; y < y0 + Math.min(h - 2, 10); y++) if ((y + tx) % 2 === 0) p.set(tx, y, T.waterD);
    });
    var r = rng(31);
    for (var i = 0; i < Math.round(h / 2.2); i++) {
      var ry = y0 + 3 + Math.round(r() * (h - 5));
      var rx = x0 + 2 + Math.round(r() * w * 0.7) + ((frame || 0) % 2 ? 1 : 0);
      var rw = Math.max(3, Math.round(w * (0.05 + r() * 0.13)));
      p.hl(rx, ry, rw, T.waterD);
      p.hl(rx + 1, ry - 1, Math.max(2, rw - 2), T.waterL);
    }
    p.hl(x0, y0 + h - 1, w, T.waterD);
    p.hl(x0, y0 + h - 2, w, T.foam);
  ...
  function world(tier, opts) {
    ...
    if (opts.fauna === true) {
      if (T.deerCount > 0) {
        ...
        if (T.deerCount > 1) deer(p, 252, MY + 2, T, false, false);
  ...
  function drawSprite(canvas, o) {
    ...
    sprite(p, grid, 1, h + 1, CHAR[o.who || 'boy']);
  ...
  function drawRoomSlot(canvas, o) {
    var W = o.W || 176, H = o.H || 132, scale = o.scale || 2, dim = o.dim == null ? 0 : o.dim;
    var p = new P(W, H);
    p.rect(0, 0, W, H, dim > 0.5 ? '#4A3524' : '#5E3D24');
    var cx = W >> 1, cy = Math.round(H * 0.62), rx = Math.round(W * 0.42), ry = Math.round(H * 0.26);
    var fa = dim > 0.5 ? '#8A6A42' : PAL.woodLight, fb = dim > 0.5 ? '#6E5334' : PAL.woodMid;
    var x, y;
    for (y = -ry; y <= ry; y++) {
      var span = Math.round(rx * (1 - Math.abs(y) / ry));
      for (x = -span; x <= span; x++) p.set(cx + x, cy + y, ((x + y) & 1) ? fb : fa);
    }
    for (y = -ry; y <= ry; y++) { var s2 = Math.round(rx * (1 - Math.abs(y) / ry)); p.set(cx - s2, cy + y, PAL.ink); p.set(cx + s2, cy + y, PAL.ink); }
    var wallH = Math.round(H * 0.36);
    var wa = dim > 0.5 ? '#B7AE9E' : PAL.wall, wb = dim > 0.5 ? '#A49A88' : '#DBD2C1';
    for (x = -rx; x <= 0; x++) {
      var top = cy - ry + Math.round(ry * (Math.abs(x) / rx)) - wallH;
      for (y = 0; y < wallH; y++) p.set(cx + x, top + y, ((x + y) & 1) ? wb : wa);
      p.set(cx + x, top - 1, PAL.ink);
    }
    for (x = 0; x <= rx; x++) {
      var top2 = cy - ry + Math.round(ry * (x / rx)) - wallH;
